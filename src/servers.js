...
const PORT =8080;
const credPath = path.join(__dirname, 'users.json');
...
const requestHandler =(req, res)=>{
  if (req.method==='GET' && req.url==='/'){
    fs.readFile(path.join(__dirname, 'public/Landing2.html'),(err, content)=>{
      if(err){
    ...
    req.on('end',()=>{
      ...
      if (user){
        res.writeHead(302, {'Location': '/DoctorSchedule'});
      }else{
  ...
  }else if(req.method=== 'GET' && req.url === '/DoctorSchedule'){
    fs.readFile(path.join(__dirname, 'public/DoctorSchedule.html'), (err, content)=>{
      if(err){
        res.writeHead(500, {'Content-Type': 'text/plain'});
        res.end('Server error');
      }else{
